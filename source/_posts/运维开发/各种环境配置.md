...
通过discord代理coze使用GPT4

```sh
docker run --name coze-discord-proxy -d --restart always \
-p 7077:7077 \
-v $(pwd)/data:/app/coze-discord-proxy/data \
-e BOT_TOKEN="MTIwOTM0MTM1NjM2Mzg4MjUwNg.GjfAJJ.P-UyJmqXvppdZvAvC0UIeg8X4felYU2bQTXg7I" \
...
-e TZ=Asia/Shanghai \
deanxv/coze-discord-proxy:latest
```

> 服务器开启clash-linux代理，使用rule代理规则，然后开启防火墙和安全组7890端口。
...
使用ls命令结合选项-l或者--size，可以查看文件的详细信息，包括文件大小。

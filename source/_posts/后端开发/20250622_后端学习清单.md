...
> 1. 学习 Java 基础和 Git 常用命令，再学习 Spring Boot 框架（学习过程中菜就多练动手实践，基础快速过一遍后再学习框架了解 Web 开发流程）。
> 2. 学习 MySQL 数据库基础常用知识，再学习 MyBatis 和 MyBatisPlus 框架（不要太依赖 plus 框架多手写 sql）。
...
graph TD
    ...
    A([开始]):::startend --> B(阶段一：Java基础与Web开发入门：how2java网站):::process
    B --> B1(Java基础语法):::process
    B --> B2(Git常用命令):::process
    B1 & B2 --> C(动手实践项目):::process
    C --> D(Spring Boot框架：柏码网站):::process
    D --> E{Web开发流程掌握}:::decision
    
    E -->|Yes| F(阶段二：数据库与持久层框架：柏码网站):::process
    F --> F1(MySQL基础):::process
    F1 --> F2(MyBatis核心):::process
    F2 --> F3[MyBatis-Plus辅助工具]:::process
    F3 --> G{使用原则}:::decision
    G -->|优先手写SQL| F2
    G -->|适当使用Plus| F3
    
    F --> H(阶段三：分布式微服务架构：柏码网站):::process
    H --> H1(Dubbo服务框架):::process
    H --> H2(Nacos注册中心):::process
    H --> H3(Redis缓存中间件):::process
    H --> H4(Seata分布式事务):::process
    H --> H5[RocketMQ消息队列]:::process
    ...
    classDef subphase fill:#E8F5E9,stroke:#4CAF50,stroke-width:2px;
    classDef component fill:#FFF3E0,stroke:#FF9800,stroke-width:2px;
    classDef note fill:#FCE4EC,stroke:#E91E63,stroke-width:2px;
    
    B:::phase -.-|包含| B1:::subphase
    B:::phase -.-|包含| B2:::subphase
    F:::phase -.-|包含| F1:::subphase
    F:::phase -.-|包含| F2:::subphase
    F:::phase -.-|包含| F3:::component
    H:::phase -.-|包含| H1:::component
    H:::phase -.-|包含| H2:::component
    H:::phase -.-|包含| H3:::component
    H:::phase -.-|包含| H4:::component
    H:::phase -.-|包含| H5:::note
```

...
可查异常，运行时异常和错误3种，其中，运行时异常和错误又叫非可查异常
...
##### 错误：Error
错误 Error，指的是系统级别的异常，通常是内存用光了，在默认设置下，一般 java 程序启动的时候，最大可以使用16m 的内存，如例不停的给StringBuffer追加字符，很快就把内存使用光了。抛出OutOfMemoryError  
...
4. e. `printStackTrace` (); 会打印出方法的调用痕迹，如此例，会打印出异常开始于 TestException 的第16行，这样就便于定位和分析到底哪里出了异常
5. 无论是否出现异常，**finally中的代码都会被执行**
...
```
#### 自定义异常
Java中的异常是通过**继承Throwable类**来实现的。因此，要**自定义异常类**，只需创建一个**继承Exception或RuntimeException的类**即可
```java
package MyCharacter;  
...
> ASCII是这样的一种码表。 只包含简单的**英文字母，符号，数字**等等。 不包含中文，德文，俄语等复杂的。
...
```
#### 编码
<span style="background:rgba(240, 200, 0, 0.2)">Java采用的是Unicode</span>
工作后经常接触的编码方式有如下几种：  
...
UNICODE 对所有字符采用 **2 个字节**浪费空间，于是出现各种减肥子编码, 比如 UTF-8对**数字和字母**就使用**一个字节**，而对**汉字**就使用**3个字节**，从而达到了减肥还能保证健康的效果，UTF-8，UTF-16和 UTF-32 针对不同类型的数据有不同的减肥效果，一般说来 UTF-8是比较常用的方式
UTF-8 编码的字节转中文：
```java
...
> `PrintWriter`:向文本输出流打印对象的格式化表示形式(Prints formatted representations of objects to a text-output stream)。PrintWriter相对于BufferedWriter的好处在于，如果PrintWriter开启了**自动刷新**，那么当PrintWriter调用println，prinlf或format方法时，输出流中的数据就会自动刷新出去。PrintWriter不但能接收字符流，**也能接收字节流**。
